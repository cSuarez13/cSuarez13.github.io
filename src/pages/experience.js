...
export default function Experience() {
  ...
  const getAllTimelineData = () => {
    ...
    if (filter === 'all' || filter === 'education') {
      data = [...data, ...experienceData.education.map(item => ({...item, type: 'education'}))];
    }
    
    if (filter === 'all' || filter === 'work') {
      data = [...data, ...experienceData.work.map(item => ({...item, type: 'work'}))];
    }
    
    // Sort by date (most recent first)
    return data.sort((a, b) => {
      const dateA = a.date.split(' - ')[1] || a.date;
      const dateB = b.date.split(' - ')[1] || b.date;
  ...
  return (
    <Layout title="Experience & Education - Claudia Suarez" description="Claudia Suarez's education and professional experience">
...
function TimelineItem({ item, index, isEducation }) {
  ...
  return (
    ...
    >
      ...
      <div className={`hidden md:block md:w-1/2 ${!isEducation ? 'md:pr-12 text-right' : 'order-2'}`}>
      ...
      <div className="flex items-center justify-center z-10 h-12 w-12 rounded-full bg-purple-100 dark:bg-purple-900 border-4 border-purple-200 dark:border-purple-800 shadow-md">
        <span className="text-purple-700 dark:text-purple-300">
          {item.icon || (isEducation ? <GraduationCap size={20} /> : <Briefcase size={20} />)}
        </span>
      </div>

      {/* Content for large screens - education on right, work on left */}
      <div className={`hidden md:block md:w-1/2 ${isEducation ? 'md:pl-12' : 'order-first'}`}>
...
function TimelineContent({ item, align = "left" }) {
  return (
    <div className="bg-white dark:bg-dark-900 p-6 rounded-lg shadow-md border border-dark-100 dark:border-dark-800 hover:shadow-lg transition-shadow duration-300">
      ...
      <h4 className="text-lg font-semibold text-dark-700 dark:text-dark-300 mb-2">{item.subtitle}</h4>
      
      <p className="text-dark-600 dark:text-dark-400 mb-4">{item.description}</p>
      
      {item.highlights && item.highlights.length > 0 && (
        <div>
          <h5 className="font-semibold text-dark-700 dark:text-dark-300 mb-2">Key Highlights:</h5>
          <ul className={`list-disc ${align === "right" ? "list-inside" : "ml-5"} text-dark-600 dark:text-dark-400 space-y-1`}>
      ...
      <div className="flex items-start">
        ...
        <div className="ml-4">
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm font-medium text-dark-500 dark:text-dark-400">{achievement.date}</span>
          </div>
